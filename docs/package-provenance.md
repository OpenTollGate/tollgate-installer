...
```
tollgate-wrt source: FreedomTechFeed/packages release — https://github.com/FreedomTechFeed/packages/releases/download/v0.6.0-alpha2-pre2/tollgate-wrt_0.6.0_alpha2_pre2_aarch64_cortex-a53.apk
```
...
```
Installed tollgate-wrt build: v0.6.0-alpha2-g7cd1882 (commit 7cd1882)
step install → "tollgate-wrt v0.6.0-alpha2-g7cd1882 (commit 7cd1882) installed via apk from FreedomTechFeed/packages release"
```

On the current main-tip package the CLI rung reports the version string with the
**source commit appended** (`v0.6.0-alpha2-g7cd1882`) while the package-metadata
rungs report the apk version (`0.6.0_alpha2_pre2-r1`) — see "Version string vs
source commit" below.
...
| Selected tag | **`v0.6.0-alpha2-pre2`** — the main-tip pre-release |
| Package version | **`0.6.0_alpha2_pre2`** (tag with the leading `v` dropped and `-` → `_`), installed as `0.6.0_alpha2_pre2-r1` |
| Source commit | **`7cd1882`** of `tollgate-module-basic-go` (the feed build is SHA-pinned to it) |
...
`feedReleaseTagDefault = "v0.6.0-alpha2-pre2"` — and one conversion rule,
...
|---|---|---|
| Package version | The feed's `PKG_VERSION`, the release tag in apk-legal spelling | `0.6.0_alpha2_pre2` (installed as `0.6.0_alpha2_pre2-r1`) |
| Binary version string | Compiled into the installed binary | `v0.6.0-alpha2-g7cd1882` |

The binary's string embeds the **source commit** (`-g7cd1882`) and does **not**
...
- The `x86_64` `.ipk` was downloaded from the release and its payload inspected:
  ...
  `v0.6.0-alpha2-g7cd1882` present in the shipped `usr/bin/tollgate-wrt` binary.
