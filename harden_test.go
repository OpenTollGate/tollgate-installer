...
func TestApkInstallFailed(t *testing.T) {
	...
	good := []string{
		...
		"(1/1) Installing tollgate-wrt (0.6.0_alpha2_pre7-r1)",
		"Executing tollgate-wrt-0.6.0_alpha2_pre7-r1.post-upgrade",
	}
	for _, s := range good {
		if apkInstallFailed(s) {
...
func TestParsePkgVersionFromApkDB(t *testing.T) {
	db := "P:busybox\nV:1.37.0-r1\n\nP:tollgate-wrt\nV:0.6.0_alpha2_pre7-r1\nA:aarch64_cortex-a53\n\nP:jq\nV:1.8.1-r1\n"
	if got := parsePkgVersionFromApkDB(db, "tollgate-wrt"); got != "0.6.0_alpha2_pre7-r1" {
		t.Errorf("parsePkgVersionFromApkDB = %q, want 0.6.0_alpha2_pre7-r1", got)
	}
...
func TestParsePkgVersionFromOpkg(t *testing.T) {
	out := "jq - 1.8.1-r1\ntollgate-wrt - 0.6.0_alpha2_pre7-r1\nnodogsplash - 5.0.2-r1\n"
	if got := parsePkgVersionFromOpkg(out, "tollgate-wrt"); got != "0.6.0_alpha2_pre7-r1" {
		t.Errorf("parsePkgVersionFromOpkg = %q, want 0.6.0_alpha2_pre7-r1", got)
	}
...
// the installed package does ("0.6.0_alpha2_pre7" vs "0.6.0_alpha2_pre7-r1").
func TestFeedPkgVersionPrefixMatchesInstalled(t *testing.T) {
	want := feedPkgVersion() // derived from feedReleaseTagDefault (pre7)
	if want == "" {
		t.Fatal("feedPkgVersion() returned empty")
	}
	if !strings.HasPrefix("0.6.0_alpha2_pre7-r1", want) {
		t.Errorf("installed version does not start with %q", want)
	}
	if strings.HasPrefix("0.6.0_alpha2_pre6-r1", want) {
		t.Errorf("pre6 must NOT match the pre7 prefix %q", want)
	}
}
